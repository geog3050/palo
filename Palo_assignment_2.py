...
def fight(participant1, participant2, first2attack):
    ...
    attack1 = participant1[3]
    attack2 = participant2[3]
    while HP1 > 0 and HP2 > 0:
        #print('None of the pokemons is dead')
        if first2attack == 1:
            HP2 -= attack_multiplier(type1, type2) * attack1
        elif first2attack == 2:
            HP1 -= attack_multiplier(type2, type1) * attack2
        rounds += 1
        if first2attack == 1:
            first2attack = 2
        elif first2attack == 2:
...
def tournament(participants):
    ...
    for i in range(0,len(participants)):
        for j in range(i + 1,len(participants)):
            homegame = fight(participants[i], participants[j],1)
            awaygame = fight(participants[i],participants[j],2)
            if homegame[0] == 1:
                wins[i] += 1
            else:
                wins[j] += 1
...
tournament(import_data('test_input.csv'))
